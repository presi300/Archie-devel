...
echo $efi >> amogus.txt
#Partition
if [ $efi -eq 4 ]; then #I will do it myself formatting
    echo "Since you chose to setup the partitions yourself, "
    echo "you have to manually select them now"
    echo "-------------------------------------------------------------------"
    ...
    mount $rootpart /mnt
    swapon $swappart
else #Auto formatting... this was a fcking pain
    swappart=$(fdisk -l $dsk | grep '4G' | cut -d' ' -f1) #Format swap
    mkswap $swappart
    efipart=$(fdisk -l $dsk | grep '300M' | cut -d' ' -f1) #Format EFI
    mkfs.fat -F32 $efipart
    touch tempdisk.txt
    fdisk -l $dsk | grep 'G' | cut -d' ' -f1 | tee -a tempdisk.txt #Format root start
    partnum=$(grep -c '/dev/' tempdisk.txt)
    if [ $partnum -eq 2 ]; then
        sed -i '1d;2d' tempdisk.txt #Yes i need to do it twice, don't bully me
    else
        echo "this is so dumb, why am i so bad at bash, someone fix this mess PLEASE"
    fi
    rootpart=$(cat tempdisk.txt)
    mkfs.ext4 $rootpart                                    #Format root end
    mount $rootpart /mnt
    mkdir -p /mnt/boot/efi
    mount $efipart /boot/efi
    swapon $swappart
    clear
    echo "Press ENTER to view changes..."
    echo "-------------------------------------------------------------------"
    read lk
    fdisk -l $dsk
    echo "-------------------------------------------------------------------"
...
touch swap.txt
echo $swappart >> swap.txt
touch efi.txt
echo $efipart >> efi.txt
touch root.txt
echo $rootpart >> root.txt
...
genfstab -U /mnt/boot/efi >> /mnt/etc/fstab
