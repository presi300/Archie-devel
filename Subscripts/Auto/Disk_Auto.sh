...
if [ $? -eq 0 ]; then #if EFI
    ...
    spacer
    cat /root/Archie/Temp_Files/userdisk.txt | grep "nvme" 
    if [ $? -eq 0 ]; then
    ...
    bash /root/Archie/Subscripts/Auto/DiskFormat/uefi.sh
else    #If not EFI
    clear
    echo "What changes will be made:"
    spacer
    ...
    spacer
    read -p "Press ENTER to proceed or do ctrl+C to exit without changes..."
    bash /root/Archie/Subscripts/Auto/DiskFormat/mbr.sh
fi
